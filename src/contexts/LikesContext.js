...
import { API_LIKES } from "../helpers/const";
...
const LikesContextProvider = (props) => {
  ...
  const addLike = async (owner, productId, rate) => {
    try {
      ...
      const response = await axios.post(API_LIKES, likes);
      getLikes(productId);
    } catch (e) {
      console.log(e);
    }
  ...
  const getLikes = async (productId) => {
    try {
      const response = await axios(API_LIKES + "?productId=" + productId);
      let action = {
        type: "GET_LIKES",
  ...
  const getLikesToEdit = async (id) => {
    try {
      const response = await axios(`${API_LIKES}/${id}`);
      let action = {
